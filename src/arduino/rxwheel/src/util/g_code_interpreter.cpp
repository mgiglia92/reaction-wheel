...
void SerialComms::processCommand(char* cmd_string)
{
    ...
    //Handshake command
    cmd = parseNumber(cmd_string, 'H', -1);
    switch(int(cmd)){
        case 0:
        default: break;
    }
    ...
    switch(int(cmd)){
        case 0: //Set PID gains
            kp = double(parseNumber(cmd_string, 'P', -1));
            ki = double(parseNumber(cmd_string, 'I', -1));
            kd = double(parseNumber(cmd_string, 'D', -1));

        case 1://Set Setpoint
            setpoint = parseNumber(cmd_string, 'Z' , -1);

        case 2: //Set Lab type
            labType = parseNumber(cmd_string, 'Y', -1);

        case 3: //Set Controller Mode (on/off)
            mode = parseNumber(cmd_string, 'M', -1);

        case 4: //Set Sample Time
            sampleTime = parseNumber(cmd_string, 'T', -1);

        case 5: //Set Output Limits
            lowerOutputLimit = parseNumber(cmd_string, 'L', -1);
            upperOutputLimit = parseNumber(cmd_string, 'U', -1);
